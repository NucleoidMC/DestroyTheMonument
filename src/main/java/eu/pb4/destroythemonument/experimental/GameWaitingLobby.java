package eu.pb4.destroythemonument.experimental;
...
public final class GameWaitingLobby {
    ...
    private List<Text> sidebarText = Collections.emptyList();
    ...
    public static GameWaitingLobby addTo(GameActivity activity, PlayerConfig playerConfig) {
        ...
        var sidebar = widgets.addSidebar(activity.getGameSpace().getSourceConfig().getName().shallowCopy().setStyle(Style.EMPTY.withColor(Formatting.GOLD).withBold(true)));
        sidebar.show();
    ...
    public void setSidebarLines(List<Text> texts) {
        this.sidebarText = List.copyOf(texts);
    }
